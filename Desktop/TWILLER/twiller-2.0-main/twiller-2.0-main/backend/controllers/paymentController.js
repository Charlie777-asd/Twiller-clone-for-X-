...
const sendSubscriptionReceipt = async (req, user, plan, amount, orderId, paymentId, expiry) => {
  try {
        ...
        const emailHtml = `
          <div style="font-family:'Outfit','Helvetica Neue',Arial,sans-serif;max-width:550px;margin:20px auto;background:#000000;border:1px solid #2f3336;border-radius:24px;overflow:hidden;box-shadow:0 20px 50px rgba(0,0,0,0.9);">
            ...
            <div style="padding:40px 32px;background:#000000;color:#e7e9ea;">
              <h3 style="color:#ffffff;font-size:20px;font-weight:800;margin:0 0 16px;text-align:center;">Thank you for your purchase, ${user.displayName}!</h3>
              ...
              <div style="background:#16181c;border-radius:16px;padding:20px;margin-bottom:24px;border:1px solid #2f3336;">
                <table style="width:100%;color:#e7e9ea;font-size:14px;border-collapse:collapse;">
                  ...
                  <tr style="border-bottom:1px solid #2f3336;"><td style="padding:10px 0;color:#71767b;">Payment ID</td><td style="padding:10px 0;text-align:right;font-family:monospace;font-size:12px;">${paymentId}</td></tr>
    ...
    if (user.phone) {
      ...
      if (sendSmsOtp) {
        const customMessage = `Twiller Premium Order Confirmed! Plan: ${planName}, Price: ₹${amount}, Order ID: ${orderId}, Payment ID: ${paymentId}, Expiry Date: ${expiryString}. Thanks for subscribing!`;
        await sendSmsOtp(user.phone, null, customMessage);
        console.log(`[Billing SMS/WhatsApp] Invoice receipt sent to ${user.phone}`);
      }
    }
  } catch (err) {
    console.error("Error sending subscription receipt notifications:", err);
  }
};
